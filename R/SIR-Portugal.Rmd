...
geo_colnames <- c("id_objeto", "id_global", "data_relatorio", "distrito",
...
geo_norte <- geo_dataset %>% filter(distrito == "+041.45756_-007.67865") %>%
  distinct_at(vars(data_relatorio), .keep_all = TRUE) %>% arrange(data_relatorio) %>%
...
cases_dataset <- cases_dataset %>%
  ...
  distinct_at(vars(data_relatorio:sintoma_difrespiratoria), .keep_all = TRUE) %>% mutate(id_objeto = 1:n())
```
...
pt_population_norte <- 3574394 # est 2018 ****** 3690681
...
population <- floor(pt_population_norte / factor)
start_inf <- 2
hospcap <- floor(pt_hospcap / factor)
...
params <- list(
  ...
  f.num = 0,
  # param.icm params
  act.rate.e = c(rep(15, 24), rep(7, steps - 24)), # number of exposure events between E and S. Otherwise as for act.rate.i.
  act.rate.i = c(rep(10, 24), rep(4, steps - 24)), # number of exposure events between I and S. It's stochastic, so the rate is an average, some individuals may have more or less. Reducing this parameter is equivalent to increasing social distancing by people in the I compartment.
...
# baseline_sim7 <- do.call("simulate", params)
...
sim_obitos <- baseline_plot_df %>% filter(compartment == "f.num") %>%
  transmute(id_objeto = time, sim_obitos = count)
sim_casos <- baseline_plot_df %>% filter(compartment %in% c("i.num", "q.num", "h.num", "f.num", "r.num")) %>%
  group_by(time) %>% summarize(sim_casos = sum(count)) %>% ungroup() %>% transmute(id_objeto = time, sim_casos)
simulados <- sim_casos %>% left_join(sim_obitos, by = "id_objeto") %>% left_join(geo_norte, by = "id_objeto")

source('../R/plot_sir.R', encoding = 'UTF-8')

plot_sir(simulados, report_rate, events, 100)
```
...
# simulados <- sim_casos %>% left_join(sim_obitos, by = "id_objeto") %>% left_join(geo_norte, by = "id_objeto")
# 
# plot(simulados$casos_confirmados/.25, type = "l", col = "red", ylim = c(0, 10000), xlim = c(0, 30), lwd = 2, main = "Comparação de dados simulados e reais", ylab = "Casos", xlab = "Dias")
# lines(simulados$n_obitos, col = "red", lwd = 2)
# lines(simulados$sim_obitos, col = "blue", lwd = 1)
# lines(simulados$sim_casos, col = "blue", lwd = 1)
# cat("MAE casos")
# sum(abs(simulados$sim_casos[1:24] - simulados$casos_confirmados[1:24]), na.rm = TRUE)
# cat("MAE obitos")
# sum(abs(simulados$sim_obitos[1:37] - simulados$n_obitos[1:37]), na.rm = TRUE)
...
complabels <- c(
  "s.num" = "Susceptible", "e.num" = "Infected/asymptomatic",
