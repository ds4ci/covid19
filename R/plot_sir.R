plot_sir <- function(dataset, report_rate, events, time) {
  ...
  lines(dataset$casos_confirmados / report_rate, col = "blue", lwd = 1)
  lines(dataset$n_obitos, col = "brown", lwd = 1)

  lines(dataset$sim_obitos, col = "brown", lwd = 1, lty = 2)
  lines(dataset$sim_casos, col = "blue", lwd = 1, lty = 2)

  abline(v = events$dates, lty = 3)
  points(events$dates, e_pos, pch = 20)
  text(events$dates, e_pos, label = events$labels, pos = 4, cex = 0.6, offset = 0.3)
  legend("topleft",
    c("Dados reais", "Simulação", "Confirmados", "Óbitos"),
    lty = c(1, 2, 0, 0),
    pch = c(-1, -1, 15, 15),
